...
public class SmartProperties {
    ...
    private static void getProperties() {
        ...
        try {
            File f = null;
            f = new File("/workspaces/properties/smartcity.properties");
            if (!f.exists()) {
                f = new File("/opt/properties/smartcity.properties");
            }
            if (!f.exists()) {
                logger.log(Level.SEVERE, "smartcity Properties File not found");
            } else {
                logger.log(Level.INFO, "smartcity Properties: {0}\n\n\n", f.getAbsolutePath());
                props.load(new FileInputStream(f));
            }
        } catch (IOException e) {
            logger.log(Level.SEVERE, "Properties file smartcity.properties not found or corrupted");
        }

    }
    private static final Logger logger = Logger.getLogger(SmartProperties.class.getName());
